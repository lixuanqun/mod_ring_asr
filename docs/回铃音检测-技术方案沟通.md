...
三种场景**共用同一识别核心**,差异只在"音频从哪来、结果往哪去、能否影响本次呼叫"。

| 维度 | 实时 | 准实时 | 数分(离线批量) |
|---|---|---|---|
| 触发 | 通话中(早期媒体) | 通话后(录音文件) | 闲时/夜间批量 |
| 时效 | 毫秒~秒 | 秒~分钟 | 小时 |
| 能否影响本次呼叫 | **能**(挂机/重路由) | 否 | 否 |
| 算力压力 | 高(在线并发) | 中(队列削峰) | 低(错峰复用) |
| 改造成本 | 高(改 FreeSWITCH) | 中(录音+队列) | 低(读历史录音) |
...
结果实时落 channel 变量(`tonedetect_tone`、`tonedetect_da_category/alias`)+ `CUSTOM tonedetect` 事件,触发 autohangup / 重路由。价值最高,对在线算力与稳定性要求最高。

### 3.2 准实时 — 录音异步识别,随话单上报

```mermaid
sequenceDiagram
  autonumber
  participant FS as FreeSWITCH(录音)
  participant MQ as 消息队列
  participant WK as 识别 Worker
  participant SVC as 检测平台
  participant CDR as 话单系统
  FS->>FS: 早期媒体录音 <uuid>.wav (recordpath)
  FS->>MQ: 投递 {uuid, wav_path, 主被叫}
  WK->>MQ: 消费任务
  WK->>SVC: 推送录音 (WS/HTTP, 复用指纹+ASR)
  SVC-->>WK: RESULT {category, alias, accuracy, score}
  WK->>CDR: 回写号码状态(随话单)
```

不影响本次呼叫,但给每条话单补齐号码状态;**队列削峰**使算力成本可控。"打标"诉求下性价比最高。
...
sequenceDiagram
  autonumber
  participant SCH as 批量调度(夜间)
  participant STORE as 历史录音库
  participant POOL as 识别 Worker 池(错峰复用)
  participant SVC as 检测平台
  participant DW as 数仓/号码库
  SCH->>STORE: 拉取待分析录音批次
  loop 批量并发
    POOL->>STORE: 读录音
    POOL->>SVC: 识别(指纹+ASR)
  ...
  POOL->>DW: 写号码状态 → 号码清洗/效果复盘
```

时效最低、成本最低、不占在线链路,适合号码清洗、营销复盘、样本库扩充。

> **结论(待确认)**:本期以 **3.1 实时**为主目标,识别核心同时为 **3.2 准实时**预留录音+队列通路;**3.3 数分**作为长期号码治理手段。
...
**建议**:实时主链路优先指纹,ASR **仅兜底**;准实时/数分可放宽用 ASR 大范围打标。开源 vs 商业按 **数据合规 + 规模成本 + 自有算力** 决策。
...
6. **(准实时预留)录音**:`recordpath` / `tonedetect_record_path` 录早期媒体为 `<uuid>.wav`,供 3.2/3.3 异步与批量识别。
...
- **平台化**:水平扩缩容、监控告警(识别耗时 / 命中率 / 各状态分布 / 队列堆积)、样本库版本管理、(准实时)队列消费 Worker。
...
| 平台化(扩缩容/监控/队列) | 后端/运维 | 中 | 并发扩缩容、监控告警、(准实时)消息队列 |
