...
### 8.5 存储与索引选型(当前规模:数百条)

> 当前样本规模为**数百条**。此规模下**不引入向量数据库 / ANN / 关系库**(过度设计、无性能收益),最优解是"目录 WAV + JSON + 内存暴力余弦",即 §12 抽象中的 `LinearIndex`(精确最近邻)。

| 数据 | 存储方式 | 说明 |
|---|---|---|
| 原始音频 | **目录 + WAV**(8k/16bit 单声道) | 文件系统或对象存储均可;参考样本**不可用有损编码**(避免损伤指纹) |
| 元数据 | **单个 `samples.json`** | name/alias/category/id/来源,几百行 |
| 特征向量 | **启动时重算,不必单独持久化** | 数百段指纹启动重算 < 1s;如需提速可缓存 `.parquet`/`.npy` |
| 索引 | **内存矩阵 + 暴力余弦**(`LinearIndex`) | 库指纹堆成 `N×D` 矩阵,查询一次矩阵乘,数百条**亚毫秒**、精确无召回损失 |

要点:
- **为何够用**:数百 × 512 维 float32 ≈ 数百 KB 常驻内存;`(N×512)·(512)` 矩阵乘 < 1ms,比 ANN 更快且精确。
- **版本/备份**:WAV + JSON 体积小,**直接用 git(或对象存储版本)管理 `samples/` 目录**,天然回滚/灰度。
- **热重载**:目录变更 → 重读 + 重算 → 原子替换内存矩阵(数百条仅数百 ms,不停服)。
- **升级触发**:当样本量增长到**约 1 万条以上**(或单段匹配延迟逼近预算)时,按 §12 把 `LinearIndex` 切换为 `HnswIndex`,**主链路与协议不变**;十万级以上再考虑 pgvector / Milvus。
